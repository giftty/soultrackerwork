...
let workbook = new Excel.Workbook();
  ...
  let driver =  await  new Builder().setChromeOptions(new chrom.Options().addArguments('--incognito'))
   ...
   let dataend= await  prompts({type:'number',name:'end', message:"Enter where to end",  validate:end=>isNaN(end)?"End is not a number ":true})
    ...
    console.log(`the follow value ${follow.foll}`) 
    if(username.usern.length>=4  && pass.password.length>=3  ){
      try{
        ...
        for (var i=datastart.start;i<=dataend.end;i++) 
        {
         ...
         await driver.wait(until.elementLocated(By.name('soul_firstname')),5000).sendKeys(worksheet.getRow(i).getCell(1).value.toString());
         await driver.wait(until.elementLocated(By.name('soul_lastname')),5000).sendKeys(worksheet.getRow(i).getCell(2).value.toString());
         await driver.wait(until.elementLocated(By.name('soul_email')),5000).sendKeys(worksheet.getRow(i).getCell(3).value.toString());
